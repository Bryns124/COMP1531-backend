...
def test_channel_user_not_part_of_channel(user_1, user_2, channel_public);
    ...
    of that channel.
    """
    user_id = decode_token(user_2['token'])['auth_user_id']
    with pytest.raises(InputError):
        assert channel_add_owner_v1(user_1['token'], channel_public['channel_id'], user_id)


def test_channel_user_already_owner(user_1, channel_public):
    """
    This test checks to see that a InputError is raised when user is already an
    owner of the channel.
    """
    user_id = decode_token(user_1['token'])['auth_user_id']
    with pytest.raises(InputError):
        assert channel_add_owner_v1(user_1['token'], channel_public['channel_id'], user_id)


def test_channel_valid_channel_and_user_not_owner():
    """
...
def test_channel_user_not_owner():
    """
    This test checks to see that an InputError is raised when removing the u_id
    of a user who is not an owner of the channel.
    """
    with pytest.raises(InputError):
        assert channel_remove_owner_v1(user_1['token'], channel_public['channel_id'], user_id)


def test_channel_only_one_owner();
    """
